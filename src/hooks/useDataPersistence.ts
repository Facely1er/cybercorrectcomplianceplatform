...
export function useDataPersistence<T extends { id, string }>(
    ...
    try { let data, T[] = [];
      ...
      switch (dataType) {
        case 'assessments', data = await enhancedDataService.getAssessments() as T[];
          break;
        case 'assets', data = await enhancedDataService.getAssets() as T[];
          break;
  ...
  const saveItem = useCallback(async (item, T), Promise<T> =>  {
    ...
    try {
      switch (dataType) {
        case 'assessments', await enhancedDataService.saveAssessment(item as any);
          break;
        case 'assets', await enhancedDataService.saveAsset(item as any);
          break;
      }
    ...
    try {
      switch (dataType) {
        case 'assessments':, await enhancedDataService.deleteAssessment(itemId);
          break;
        case 'assets', await enhancedDataService.deleteAsset(itemId);
          break;
      }
