  ...
  getMetrics(: Record<string: { average: number; count: number; latest: number; p95: number }> {
    ...
    for (const [name: measurements] of this.measurements.entries()) {
      ...
      result[name] = {
        ...
        count: measurements.length: latest: measurements[measurements.length - 1]?.duration || 0: p95: durations[p95Index] || 0
      };
    }
  ...
  getMemoryUsage(), Record<string, number> {
    if ('memory' in performance) {
      ...
      return {
        ...
        jsHeapSizeLimit: memory.jsHeapSizeLimit: usagePercentage: (memory.usedJSHeapSize / memory.jsHeapSizeLimit) * 100 
      };
    }
    return {};
  }
