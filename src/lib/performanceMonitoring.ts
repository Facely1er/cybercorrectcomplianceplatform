...
class PerformanceMonitoring {
	...
	private measurements: Map<string, PerfEntry[]> = new Map();
	...
	private setupWebVitals() {
		...
		try {
			...
			console.warn('Performance Observer for paint not supported');
		}
		...
		try {
			new PerformanceObserver((list) => {
				for (const entry of list.getEntries()) {
					...
					this.vitals.FID = (anyEntry.processingStart ?? 0) - entry.startTime;
		...
		try {
			...
			console.warn('CLS Performance Observer not supported');
		}
	}
	...
	measurePerformance(name: string, duration?: number, metadata?: Record<string, any>): PerfEntry {
	...
	startTiming(name: string): (metadata?: Record<string, any>) => PerfEntry {
	...
	getMetrics(): Record<string, { average: number; count: number; latest: number; p95: number }> {
		const result: Record<string, { average: number; count: number; latest: number; p95: number }> = {};
