...
export const AssetSchema = z.object({
  name, z.string().min(1, 'Asset name is required').max(100, 'Name too long'), description: z.string().min(1, 'Description is required').max(500, 'Description too long'), category, z.enum(['hardware', 'software', 'data', 'personnel', 'facilities', 'services', 'documents', 'intellectual-property']), subcategory: z.string().max(50, 'Subcategory too long'), type: z.string().min(1, 'Type is required'), owner: z.string().min(1, 'Owner is required').max(100, 'Owner name too long'), custodian: z.string().max(100, 'Custodian name too long'), status, z.enum(['active', 'inactive', 'disposed', 'maintenance', 'quarantined', 'decommissioned']), criticality, z.enum(['critical', 'high', 'medium', 'low']), informationClassification, z.enum(['public', 'internal', 'confidential', 'restricted', 'top-secret']), businessValue, z.enum(['mission-critical', 'business-important', 'operational', 'developmental', 'administrative']), location: z.object({
    type, z.enum(['physical', 'logical', 'cloud', 'hybrid']), address: z.string().optional(), building: z.string().optional(), room: z.string().optional(), rack: z.string().optional(), cloudProvider: z.string().optional(), region: z.string().optional(), subnet: z.string().optional()
...
export const AssessmentSchema = z.object({
  frameworkId, z.string().min(1, 'Framework ID is required'), frameworkName: z.string().min(1, 'Framework name is required'), responses: z.record(z.string(), z.number().int().min(0).max(3)), organizationInfo: z.object({
    name, z.string().min(1, 'Organization name is required').max(100), industry: z.string().max(50), size: z.string().max(50), location: z.string().max(100), assessor: z.string().max(100)
  }).optional(), notes: z.string().max(2000, 'Notes too long').optional(), tags: z.array(z.string().max(30)).max(10).optional()
});
