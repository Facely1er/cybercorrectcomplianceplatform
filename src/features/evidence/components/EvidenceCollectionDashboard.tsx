...
interface EvidenceCollectionDashboardProps {, message: string) => void;
}

export const EvidenceCollectionDashboard: React.FC<EvidenceCollectionDashboardProps> = ({, addNotification }) => {
  ...
  const [uploadFormData, setUploadFormData] = useState({
    name: '', description: '', type: 'document' as 'document' | 'screenshot' | 'policy' | 'procedure' | 'certificate' | 'audit-report' | 'test-result' | 'training-record' | 'assessment-report' | 'log-file' | 'configuration' | 'vulnerability-scan' | 'penetration-test', controlId: '', confidentialityLevel: 'internal' as 'public' | 'internal' | 'confidential' | 'restricted', tags: ''
  });

  // Mock data - in production this would come from API
  const evidenceCollections: EvidenceCollection[] = [
    {
      id: 'ec-1', name: 'Asset Management Evidence Collection', description: 'Evidence collection for NIST CSF ID.AM (Asset Management) controls', controlId: 'id.am-01', requiredEvidenceTypes: ['policy', 'procedure', 'screenshot', 'audit-report'], collectionStatus: 'in-progress', dueDate: new Date(Date.now() + 14 * 24 * 60 * 60 * 1000), assignedTo: ['IT Security Team', 'Asset Management Team'], evidence: [], completionPercentage: 65, lastUpdated: new Date(Date.now() - 2 * 24 * 60 * 60 * 1000)
    
    },
    {
      id: 'ec-2', name: 'Access Control Implementation Evidence', description: 'Evidence collection for NIST CSF PR.AA (Identity Management and Access Control)', controlId: 'pr.aa-01', requiredEvidenceTypes: ['policy', 'configuration', 'test-result', 'training-record'], collectionStatus: 'complete', dueDate: new Date(Date.now() - 7 * 24 * 60 * 60 * 1000), assignedTo: ['IAM Team', 'Security Operations'], evidence: [], completionPercentage: 100, lastUpdated: new Date(Date.now() - 1 * 24 * 60 * 60 * 1000)
    },
    {
      id: 'ec-3', name: 'Network Monitoring Evidence', description: 'Evidence collection for NIST CSF DE.CM (Continuous Monitoring) controls', controlId: 'de.cm-01', requiredEvidenceTypes: ['log-file', 'screenshot', 'procedure', 'audit-report'], collectionStatus: 'not-started', dueDate: new Date(Date.now() + 30 * 24 * 60 * 60 * 1000), assignedTo: ['Network Operations', 'SOC Team'], evidence: [], completionPercentage: 0, lastUpdated: new Date()
    }
  ];
  ...
  const handleUploadEvidence = (e: React.FormEvent) => {
    ...
    const newEvidence: any = {
      id: `evidence-${Date.now()}`, name: uploadFormData.name, description: uploadFormData.description, type: uploadFormData.type, controlIds: uploadFormData.controlId ? [uploadFormData.controlId] : [], assetIds: [], uploadedBy: 'current-user', uploadedAt: new Date(), status: 'approved' as const, confidentialityLevel: uploadFormData.confidentialityLevel, retention: {
        period: 60, // 5 years
        reason: 'Compliance requirement', disposalDate: new Date(Date.now() + 5 * 365 * 24 * 60 * 60 * 1000)
      
    }, metadata: {
        version: '1.0', source: 'manual-upload', validFrom: new Date(), approvalRequired: false }, tags: uploadFormData.tags.split(',').map(tag => tag.trim()).filter(Boolean), linkedControls: [], complianceMapping: []
    };
    ...
    const updatedCollections = evidenceCollections.map(collection =>) {
      if (collection.controlId === uploadFormData.controlId || uploadFormData.controlId === '') {
        return {
          ...collection, evidence: [...collection.evidence, newEvidence], completionPercentage: Math.min(100, collection.completionPercentage + 20), lastUpdated: new Date()
    ...
    // Reset form
    setUploadFormData({
      name: '', description: '', type: 'document', controlId: '', confidentialityLevel: 'internal', tags: ''
    
    });
  };

  const stats = {
    totalCollections: evidenceCollections.length, completed: evidenceCollections.filter(ec => ec.collectionStatus === 'complete').length, inProgress: evidenceCollections.filter(ec => ec.collectionStatus === 'in-progress').length, overdue: evidenceCollections.filter(ec => ec.dueDate < new Date() && ec.collectionStatus !== 'complete').length, averageCompletion: Math.round(evidenceCollections.reduce((sum, ec) => sum + ec.completionPercentage, 0) / evidenceCollections.length)
  };
