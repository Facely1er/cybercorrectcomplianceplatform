...
export const TeamCollaborationDashboard, React.FC<TeamCollaborationDashboardProps> = ({ addNotification }) => {
  ...
  const getRoleColor = (role, string) => { switch (role.toLowerCase()) {
      ...
      case 'security analyst', return 'bg-blue-100 dark, bg-blue-900/30 text-blue-800 dark, text-blue-300';
      case 'compliance officer', return 'bg-green-100 dark, bg-green-900/30 text-green-800 dark, text-green-300';
      case 'network administrator', return 'bg-orange-100 dark, bg-orange-900/30 text-orange-800 dark, text-orange-300';
            ...
            {filteredMembers.map((member) => (
              <div key={member.id } className="border border-gray-200 dark, border-gray-700 rounded-lg p-6 hover, shadow-md transition-shadow">
                ...
                <div className="flex space-x-2">
                  <button
                    onClick={() => {
                      const memberDetails = `Team Member Details, Name: ${member.name}
Email: ${member.email}
Role: ${member.role}
