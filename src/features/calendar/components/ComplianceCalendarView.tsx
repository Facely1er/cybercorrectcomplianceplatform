...
export const ComplianceCalendarView: React.FC<ComplianceCalendarViewProps> = ({ addNotification }) => {
  ...
  const [eventFormData, setEventFormData] = useState<{ title: string; description: string; type: CalendarEventType; priority: 'low' | 'medium' | 'high' | 'critical'; startDate: string; endDate: string; allDay: boolean; location: string; attendees: string }>(
    {
      title: '',
      description: '',
      type: 'assessment',
  ...
  const handleCreateEvent = (e: React.FormEvent) => {
    ...
    const newEvent: CalendarEvent = {
      id: `event-${Date.now()}`, title: eventFormData.title, description: eventFormData.description, type: eventFormData.type, startDate: new Date(eventFormData.startDate), endDate: eventFormData.endDate ? new Date(eventFormData.endDate) : undefined, allDay: eventFormData.allDay, priority: eventFormData.priority, status: 'scheduled', attendees: eventFormData.attendees.split(',').map(email => ( {
