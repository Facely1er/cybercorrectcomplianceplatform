...
export const ControlsManagementView: React.FC<ControlsManagementViewProps> = ({
    ...
    const controlData: Partial<Control> = {
      ...
      implementation: {
        ...
        metrics: [], alerting: {
          ...
          dataCollection: { automated: false, sources: [], frequency: 'manual' }, analysis: { automated: false, algorithms: [], ml_enabled: false }, response: { automated: false, actions: [] }
        }
      }, evidence: [], policies: [], assets: [], dependencies: [], effectiveness: {
        implementationScore: 0, operationalScore: 0, complianceScore: 0, costEffectiveness: 0, riskReduction: 0, maturityLevel: 1, lastMeasured: new Date(), trend: 'stable' as const }, costs: {
        implementation: { capital: 0, operational: 0, timeline: 'TBD' }, maintenance: { annual: 0, resources: [] }, testing: { frequency: 'quarterly', cost: 0, resources: [] }, training: { initial: 0, ongoing: 0, frequency: 'annually' }
      }, risks: [], exceptions: []
    };
  ...
  // Enhanced statistics
  const stats = {
    total: controls.length, operational: controls.filter(c => c.status === 'operational').length, implemented: controls.filter(c => c.status === 'implemented').length, inProgress: controls.filter(c => c.status === 'in-progress').length, notImplemented: controls.filter(c => c.status === 'not-implemented').length, avgEffectiveness: controls.length > 0 ? Math.round(controls.reduce((sum, c) => sum + c.effectiveness.implementationScore, 0) / controls.length) : 0, highPriority: controls.filter(c => c.priority === 'critical' || c.priority === 'high').length, costEffectiveControls: controls.filter(c => c.effectiveness.costEffectiveness >= 80).length 
    };
