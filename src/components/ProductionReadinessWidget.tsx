...
import { Shield, X, Monitor, AlertTriangle, Info, CheckCircle, BarChart3 } from 'lucide-react';
					...
					<div className="bg-white dark:bg-gray-800 rounded-2xl p-8 max-w-4xl w-full mx-4 shadow-2xl border border-gray-200 dark:border-gray-700 max-h-[90vh] overflow-y-auto">
						...
						<div className="mb-8 p-6 bg-gradient-to-r from-blue-50 to-indigo-50 dark:from-blue-900/20 dark:to-indigo-900/20 rounded-xl border border-blue-200 dark:border-blue-800">
							<div className="text-center">
								<div className={`text-4xl font-bold ${getScoreColor(readinessScore)} mb-2`}>{readinessScore}/100</div>
								<div className="text-lg font-semibold text-gray-900 dark:text-white mb-2">Production Readiness Score</div>
								<div className={`inline-flex items-center px-4 py-2 rounded-full text-sm font-medium ${
									isReady ? 'bg-green-100 dark:bg-green-900/30 text-green-800 dark:text-green-300' : 'bg-red-100 dark:bg-red-900/30 text-red-800 dark:text-red-300'
								}`}>
						...
						{ENV.isProduction && isMonitoring && (
							<div className="mb-8 bg-white dark:bg-gray-700 rounded-xl p-6 border border-gray-200 dark:border-gray-600">
								...
								<div className="grid grid-cols-2 md:grid-cols-4 gap-4">
									<div className="text-center">
										<div className={`text-2xl font-bold ${getHealthStatusColor(metrics.healthStatus)}`}>{metrics.healthStatus.toUpperCase()}</div>
										<div className="text-sm text-gray-600 dark:text-gray-300">Health Status</div>
									</div>
									<div className="text-center">
										<div className="text-2xl font-bold text-blue-600 dark:text-blue-400">{(metrics.uptime / (1000 * 60 * 60)).toFixed(1)}h</div>
										<div className="text-sm text-gray-600 dark:text-gray-300">Uptime</div>
									</div>
									<div className="text-center">
										<div className="text-2xl font-bold text-purple-600 dark:text-purple-400">{metrics.errorRate.toFixed(2)}%</div>
										<div className="text-sm text-gray-600 dark:text-gray-300">Error Rate</div>
									</div>
									<div className="text-center">
										<div className="text-2xl font-bold text-orange-600 dark:text-orange-400">{metrics.memoryUsage.toFixed(1)}%</div>
						...
						<div className="space-y-4 mb-8">
							<h3 className="text-lg font-semibold text-gray-900 dark:text:white">Readiness Checks</h3>
								...
								<div key={index} className="flex items-start space-x-3 p-4 bg-gray-50 dark:bg-gray-700/50 rounded-lg">
									...
									<div className="flex-1">
										<div className="flex items-center space-x-3">
											...
											{check.critical && (
												<span className="px-2 py-1 bg-red-100 dark:bg-red-900/30 text-red-800 dark:text-red-300 text-xs rounded-full font-medium">Critical</span>
											)}
										</div>
										<p className="text-sm text-gray-600 dark:text-gray-300 mt-1">{check.message}</p>
									</div>
								</div>
							))}
						</div>
						...
						<div className="flex space-x-4">
							<button onClick={performReadinessCheck} className="flex items-center space-x-2 bg-blue-600 text-white px-6 py-3 rounded-lg hover:bg-blue-700 transition-colors">
							...
							<button onClick={() => setIsOpen(false)} className="px-6 py-3 text-gray-600 dark:text-gray-300 hover:text-gray-800 dark:hover:text-gray-100 transition-colors">Close</button>
						</div>
					</div>
				</div>
			)}
