...
export class ErrorBoundary extends Component<Props, State> {
  ...
  render() {
    if (this.state.hasError) {
      ...
      return (
        <div data-error-boundary className="min-h-screen flex items-center justify-center bg-gray-50 dark:bg-gray-900 px-4">
          <div className="max-w-lg w-full bg-white dark:bg-gray-800 rounded-xl shadow-lg border border-gray-200 dark:border-gray-700 p-8 text-center">
            ...
            {this.props.showErrorDetails && ENV.isDevelopment && this.state.error && (
              <details className="mb-6 text-left bg-gray-100 dark:bg-gray-700 rounded-lg p-4">
                <summary className="cursor-pointer text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Error Details (Development Mode)</summary>
            ...
            {this.state.errorId && (
              <p className="text-xs text-gray-500 dark:text-gray-400 mt-6">Error ID: {this.state.errorId}</p>
            )}

            {ENV.isProduction && (
              <div className="mt-6 p-4 bg-blue-50 dark:bg-blue-900/20 rounded-lg border border-blue-200 dark:border-blue-800">
                <p className="text-sm text-blue-800 dark:text-blue-200">
                  Our monitoring systems have been automatically notified. If this issue persists, please contact support with the Error ID above.
                </p>
              </div>
            )}
          </div>
...
export const withErrorBoundary = <P extends object>(
  ...
  const WrappedComponent = (props: P) => (
    <ErrorBoundary fallback={errorFallback} onError={onError} showErrorDetails={ENV.isDevelopment}>
