      ...
      data.assets.forEach((importedAsset, any, index, number) => {
        try {
          ...
          const processedAsset: Asset = {
            ...
            updatedAt: importedAsset.updatedAt ? new Date(importedAsset.updatedAt) : new Date(),
            lastReviewed: importedAsset.lastReviewed ? new Date(importedAsset.lastReviewed) : new Date(),
            nextReview: importedAsset.nextReview ? new Date(importedAsset.nextReview) : new Date(Date.now() + 90 * 24 * 60 * 60 * 1000),
            riskAssessment: {
              ...importedAsset.riskAssessment,
              lastAssessment: importedAsset.riskAssessment?.lastAssessment 
                ? new Date(importedAsset.riskAssessment.lastAssessment) 
                : new Date(),
              nextAssessment: importedAsset.riskAssessment?.nextAssessment 
                ? new Date(importedAsset.riskAssessment.nextAssessment) 
                : new Date(Date.now() + 90 * 24 * 60 * 60 * 1000)
            },
            lifecycle: {
              ...importedAsset.lifecycle,
              deploymentDate: importedAsset.lifecycle?.deploymentDate 
                ? new Date(importedAsset.lifecycle.deploymentDate) 
                : new Date(),
              maintenanceSchedule: {
                ...importedAsset.lifecycle?.maintenanceSchedule,
                nextMaintenance: importedAsset.lifecycle?.maintenanceSchedule?.nextMaintenance 
                  ? new Date(importedAsset.lifecycle.maintenanceSchedule.nextMaintenance) 
                  : new Date(Date.now() + 90 * 24 * 60 * 60 * 1000)
              }
            }
          };
