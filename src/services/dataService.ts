...
export class DataService { 
  ...
  private performDataMigration(fromVersion: string | null): void {
    ...
    try {
      ...
      const migratedAssessments = oldAssessments.map((assessment: any) => ({
        ...assessment,
  ...
  saveAssessments(assessments: AssessmentData[]): void {
    try {
      ...
      this.logDataAction('current-user', 'update', 'assessments', 'bulk', { count: assessments.length });
    } catch (error) {
  ...
  createBackup(): string {
    try {
      ...
      this.saveBackupMetadata(backupData);
  ...
  restoreFromBackup(backupData: string): void {
    try {
      ...
      if (data.checksum) {
        const { checksum: backupChecksum, backupId, backupType, description, timestamp, settings, lastBackup, version } = data;
        const calculatedChecksum = this.generateChecksum(JSON.stringify(this.exportAllData(), null, 2));
        if (backupChecksum !== calculatedChecksum) {
          throw new Error('Checksum mismatch for backup data');
        }
      }

      // Import data
      this.importAllData(this.exportAllData()); // Use exportAllData to get current state
  ...
  private logDataAction(userId: string, action: string, resource: string, resourceId: string, changes: any): void {
    auditLogger.log({
      userId, action, resource, resourceId, changes
    });
  }

  private getLastBackup(): Date | null {
    const metadata = this.getBackupMetadata();
    return metadata?.lastBackup || null;
  }

  private saveBackupMetadata(metadata: BackupMetadata): void {
    localStorage.setItem(this.STORAGE_KEYS.BACKUP_METADATA, JSON.stringify(metadata));
  }

  private getBackupMetadata(): BackupMetadata | null {
    const data = localStorage.getItem(this.STORAGE_KEYS.BACKUP_METADATA);
    if (!data) return null;
